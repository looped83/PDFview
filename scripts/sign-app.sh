#!/usr/bin/env bash
#
# sign-app.sh — signs dist/PDFViewer.app with a "Developer ID Application" identity and
...
# but PDFViewer.app embeds no third-party frameworks or plugins — only its own single
# executable — so there is nothing nested for --deep to sign incorrectly here.
log "Signing PDFViewer.app (Hardened Runtime, entitlements, secure timestamp)"
